...
def is_market_open():
    ...
    return trading_hours.open_kst <= now_kst <= trading_hours.close_kst
...
def stock_chart_data(request):
    ...
    try:
        ...
        df = stock.get_market_ohlcv_by_date(start.strftime("%Y%m%d"), end.strftime("%Y%m%d"), stock_code)
